...
import { mintImportToken, getMe } from "@/lib/api";
...
// A subtle, secondary confidence check — NOT a main CTA. Rendered as a plain
// text link below the steps so it doesn't compete with "Add the shortcut".
function CheckIt(): JSX.Element {
  ...
  async function onCheck() {
    ...
    if (res.ok && res.data.recent.length > 0) {
      ...
      setResult(`We see your latest: ${r.gameId} ${formatResult(r.gameId, r.value, r.solved, r.detail)}`);
    } else {
      setResult(null);
    }
  }
  return (
    <div className={styles.checkRow}>
      <button type="button" className={styles.linkBtn} onClick={onCheck}>Check that it worked</button>
      {checked && (result
        ? <span className={styles.ok}>✓ {result}</span>
